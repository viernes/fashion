...
class sale_order_line(models.Model):
    ...
    ad_is_model_variant = fields.Boolean(default=True)
    ...
    def _prepare_order_line_procurement(self, group_id=False):
        ...
        res = super(sale_order_line, self)._prepare_order_line_procurement(group_id=group_id)
...
class sale_order(models.Model):
                ...
                for var_val in variant.attribute_value_ids:
                    if val.id == var_val.id:
                        ...
                        if not res.get('value', False):
                            raise Warning(_("Problem loading some values for a product."))
        ...
        response = {
            'measurebar': {
                ...
                'models': [{
                    ...
                    'discount': 0,
                    'image': image,
                    'taxes': ' ,'.join(map(lambda x: x.name, product_template.taxes_id)),
    ...
    def new_row_defaults(self):
        product_template_id = self.env.context.get('product_template_id', False)
        if not product_template_id:
            raise Warning("No product template given to use.")

        product_template = self.env['product.template'].browse(product_template_id)
        all_vals = self.env['product.attribute.value'] \
            .search([('attribute_id', '=', product_template.model_attribute.id)])
    ...
    def button_dummy(self):
        with api.Environment.manage():
            with registry(self.env.cr.dbname).cursor() as new_cr:
                ...
                for line in self.with_env(new_env).order_line:
                    if line.product_uom_qty <= 0:
                        self.with_env(new_env).order_line -= line
                new_env.cr.commit()

        return super(sale_order, self).button_dummy()
